#!/bin/bash
# 实验E：+ Edge + Frequency + Perceptual（完整版）
# 目的：验证加入感知损失的额外收益（计算量大，需权衡）

CUDA_VISIBLE_DEVICES="6,7" accelerate launch --num_processes=2 --main_process_port 12349 train_seesr.py \
--pretrained_model_name_or_path="preset/models/stable-diffusion-2-base" \
--output_dir="./experience/consistency_full" \
...
--consistency_perceptual_loss_type l1 \
--consistency_high_freq_weight 2.0

# 完整版：启用所有一致性损失（包括VGG感知损失）
# 注意：感知损失会增加显存和计算时间约30%
